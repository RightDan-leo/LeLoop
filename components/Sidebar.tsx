  ...
  return (
    <aside className="w-64 bg-slate-900 border-r border-slate-700 flex flex-col z-20 shadow-xl">
      <div className="p-4 border-b border-slate-700">
        ...
        <p className="text-slate-400 text-xs mt-1">{t('sidebar.subtitle')}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-emerald-100">{t('nodes.source.label')}</div>
              <div className="text-xs text-slate-400">{t('nodes.source.desc')}</div>
            </div>
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-blue-100">{t('nodes.pool.label')}</div>
              <div className="text-xs text-slate-400">{t('nodes.pool.desc')}</div>
            </div>
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-orange-100">{t('nodes.converter.label')}</div>
              <div className="text-xs text-slate-400">{t('nodes.converter.desc')}</div>
            </div>
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-red-100">{t('nodes.drain.label')}</div>
              <div className="text-xs text-slate-400">{t('nodes.drain.desc')}</div>
            </div>
          </div>
        </div>

        <div
          className="bg-slate-800 p-3 rounded-lg border border-slate-700 cursor-grab hover:border-indigo-500 hover:bg-slate-750 transition-all group"
          onDragStart={(event) => onDragStart(event, NodeType.SPLITTER)}
          draggable
        >
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded bg-indigo-900/50 border border-indigo-500 flex items-center justify-center text-indigo-400 transform rotate-180">
              <div className="transform rotate-180"><Split size={16} /></div>
            </div>
            <div>
              <div className="text-sm font-semibold text-indigo-100">{t('nodes.splitter.label')}</div>
              <div className="text-xs text-slate-400">{t('nodes.splitter.desc')}</div>
            </div>
          </div>
        </div>

        <div
          className="bg-slate-800 p-3 rounded-lg border border-slate-700 cursor-grab hover:border-violet-500 hover:bg-slate-750 transition-all group"
          onDragStart={(event) => onDragStart(event, NodeType.MERGER)}
          draggable
        >
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded bg-violet-900/50 border border-violet-500 flex items-center justify-center text-violet-400">
              <Merge size={16} />
            </div>
            <div>
