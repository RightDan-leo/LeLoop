...
export const PropertiesPanel: React.FC<PropertiesPanelProps> = ({
        ...
        return (
            <div className="bg-slate-800/50 p-3 rounded border border-slate-700/50 space-y-3">
                <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-300">数值设定</span>
                    <label className="flex items-center gap-2 cursor-pointer group">
                        <input
                            ...
                            className="w-3 h-3 rounded border-slate-600 text-blue-500 focus:ring-offset-0 focus:ring-1 focus:ring-blue-500 bg-slate-700"
                        />
                ...
                {!data.isRandom ? (
                    <div>
                        <label className="block text-[10px] font-medium text-slate-400 mb-1">{labels.single}</label>
                        <input
                            type="number"
                            value={data.rate}
                    ...
                    <div className="flex gap-2">
                        <div className="flex-1">
                            <label className="block text-[10px] font-medium text-slate-400 mb-1">{labels.min}</label>
                            <input
                                type="number"
                                value={data.rate}
                                onChange={(e) => onChange('rate', parseFloat(e.target.value))}
                        ...
                        <div className="flex-1">
                            <label className="block text-[10px] font-medium text-slate-400 mb-1">{labels.max}</label>
                            <input
                                type="number"
    ...
    const renderContent = () => {
        if (selectedNode) {
            ...
            return (
                <div className="space-y-4">
                    <div>
                        ...
                        {isText ? (
                            <textarea
                                ...
                                className="w-full bg-slate-800 border border-slate-600 rounded px-2 py-1.5 text-sm text-white focus:ring-1 focus:ring-blue-500 focus:outline-none min-h-[80px]"
                    ...
                    {isText && (
                        <div>
                            <label className="block text-xs font-medium text-slate-400 mb-1">{t('properties.font_size') || 'Font Size'} ({selectedNode.data.fontSize || 16}px)</label>
                            <input
                                type="range"
                                min="12"
                                max="64"
                    ...
                    {isPool && (
                        <>
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-1">当前值</label>
                                <input
                                    type="number"
                            ...
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-1">最大容量 (可选)</label>
                                <input
                                    type="number"
                    ...
                    {isConverter && (
                        <>
                            ...
                            <p className="text-[10px] text-slate-500 mt-2 p-2 bg-slate-800/50 rounded border border-slate-700">
        ...
        if (selectedEdge) {
            return (
                <div className="space-y-4">
                    <div className="p-2 bg-blue-900/20 border border-blue-800 rounded mb-4">
                    ...
                    <div className="pt-2 border-t border-slate-700">
                        <p className="text-[10px] text-slate-500">Edge ID: {selectedEdge.id}</p>
                    </div>
                </div>
            );
        }
